...
Instead of guessing a delay, I built a retry loop with exponential backoff: after creating the store, the app tries to fetch it. If the fetch fails because the record isn't available yet, it waits briefly and tries again, up to a fixed number of attempts.
